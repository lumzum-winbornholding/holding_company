...
function populate_from_funds_hold(frm) {
	frappe.call({
		method: 'frappe.client.get',
		args: {
			doctype: 'Funds Hold',
			name: frm.doc.funds_hold
		},
		callback: function(r) {
			if (r.message) {
				const funds_hold = r.message;
				
				// Fill fields from Funds Hold
				frm.set_value('transaction_fee', funds_hold.transaction_fee);
				frm.set_value('transaction_fee_vat', funds_hold.transaction_fee_vat);
				frm.set_value('net_amount', funds_hold.net_amount);
				
				frappe.show_alert(__('Fields populated from Funds Hold'), 3);
			}
		}
	});
}
				...
				frappe.call({
					...
					callback: function(mop_r) {
						if (mop_r.message && mop_r.message.accounts) {
							...
							if (company_account) {
								// Map the fields from Mode of Payment Account to Funds Payout
