...
class FundsCallback(Document):
	...
	def create_journal_entry(self):
		"""Create journal entry for funds callback transaction"""
		...
		je.user_remark = f"Funds callback for original Payment Entry: {self.payment_entry}"
		
		# Get company abbreviation for VAT account
		company_abbr = frappe.db.get_value("Company", self.company, "abbr")
		vat_account = f"VAT (Assets) - {company_abbr}"
		
		# Detect origin based on field logic
		# If funds_payout is filled, origin is from Funds Payout
		# If only funds_hold is filled (funds_payout is empty), origin is from Funds Hold
		if self.funds_payout:
			# Created from Funds Payout
			# Credit gross to bank_account
			je.append("accounts", {
				"account": self.bank_account,
				"credit_in_account_currency": self.gross_amount
			})
			
			# Debit gross to loss_account
			je.append("accounts", {
				"account": self.loss_account,
				"debit_in_account_currency": self.gross_amount
			})
			
		else:
			# Created from Funds Hold (funds_hold is filled but funds_payout is not)
			# Credit gross to hold_account
			je.append("accounts", {
				"account": self.hold_account,
				"credit_in_account_currency": self.gross_amount
			})
			
			# Credit fee to bank_account
			...
			# Debit fee to transaction_fee_account
			if self.transaction_fee:
				je.append("accounts", {
	...
	def cancel_journal_entry(self):
		...
		if self.journal_entry:
			try:
				...
				frappe.log_error(
					message=f"Could not cancel Journal Entry for Funds Callback document {self.name}: {e}",
					title="Funds Callback Cancellation Script Error"
				)